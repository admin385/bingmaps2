// Additions for Marker Index  22Apr25, 29Apr25, 02May25

const deg_R = 180/Math.PI;
const CP = ["N","NE","E","SE","S","SW","W","NW"];  // compass points
const U = ["\u2191","\u2197","\u2192","\u2198","\u2193","\u2199","\u2190","\u2196"];  // compass arrows

var Axx,Ayy, Q; //NB: globals, now
//var qX,qY;  // ADD 02May25 KLUDGE, for SLee: tile digits from clickMarker, showMarker

function zoomTo(r) { //console.log("zoomTo "+r+" "+typeof r)
var c = map.getCenter(), g, z = map.getZoom();
	...
	else {g = markers[r].geometry; map.setView({center:new Microsoft.Maps.Location(g.y, g.x), zoom:19})}; //console.log(markers[r].geometry)
}

function addMarkerRow(qxxyy, u) { //console.log("add "+qxxyy+"; "+Axx+Ayy); //MOD 02May25
var a, b, c, d, dx,dy, i,i0, j,j0, p, q,q0, t, xx, yy, XY;  // MOD 02May25
...
//t = (qxxyy.length==4 ? "5" : "") + qxxyy;
	...
	dx = xx-Axx; d = Math.abs(dx); i = 0; if (Axx>=50) {if (dx+100<d) i = +1} else {if (100-dx<d) i = -1}; if (i) dx = dx +i*100;  // in-tile offsets
	dy = yy-Ayy; d = Math.abs(dy); j = 0; if (Ayy>=50) {if (dy+100<d) j = +1} else {if (100-dy<d) j = -1}; if (j) dy = dy +j*100;
console.log(q+" "+i+" "+j+"; "+xx+" "+yy+" "+dx+" "+dy)
	i0 = i; j0 = j; q0 = 5 +i0 -3*j0;  //ADD 02May25: Note nearest tile
  //if (isNaN(q)) q1 = (5 +i -3*j);  //DEL 02May25 of nearest tile, for @xxyy 
  //else { i1 = ((q-1)%3) -1;  j1 = 1 -Math.trunc((q-1)/3); //DEL 29Apr25 if (i1==i && j1==j) return;
  //dx += 100*(i1-i); dy += 100*(j1-j); }  // for explicit tile offsets
	if (!isNaN(q)) { // REP above 02May25
console.log(q+"?"+q0)
	  if (q==q0 && q0==5) return false;  // skip nearest tile iff not anchor tile, during [More]
	  i = ((q-1)%3) -1;  j = 1 -Math.trunc((q-1)/3);
	  dx += 100*(i-i0); dy += 100*(j-j0);  // additional full tile offsets, NB: in decameters
	}
console.log(qxxyy.charAt(0)+"?"+q+" "+i+" "+j+"; "+xx+" "+yy+" "+dx+" "+dy)
...
r = tbl.rows.length; //console.log(markers[r-1].geometry)
p = r; while (--p>0) if (d >= parseInt(tbl.rows[p].cells[1].innerHTML)) break;
XY = (isNaN(q) ? "" : u.Eing.toString().slice(-4,-3)+u.Ning.toString().slice(-4,-3));  //ADD 05May25
row = tbl.insertRow(++p) //WAS: (-1) // for end
//row.insertCell(0).innerHTML = "<button onclick='zoomTo("+r+")'><b>"+qxxyy+"</b></button>";
row.insertCell(0).innerHTML = "<button onclick='zoomTo("+r+")'><b>"+XY+"@"+qxxyy.slice(-4)+"</b></button>";  //KLUDGE, ADD 02May25 for SLee
row.insertCell(1).innerHTML = d.toFixed(1);
row.insertCell(2).innerHTML = b.toFixed(0);
row.insertCell(3).innerHTML = CP[c]; //U[r%8]
row.insertCell(4).innerHTML = U[c]; //U[r%8]
return (q==q0 ? false :true);
}
...
var lastCode, lastUTM={};  //31Oct add: support [More], globals
 ...
 uPosn = toUTM(POSN); console.log("Anchor: "+uPosn.Eing+"_"+uPosn.Ning) // initial anchor
//CLL = toLL(uPosn, uPosn.Zone); //del 25Aug; add 24Aug: alert(CLL.lng.toFixed(6)+"&"+CLL.lat.toFixed(6))
 ...
 lastCode = Mxxyy+"."; lastUTM = uPosn; markers.length = 0; //ADD 29Apr25 support [More]
...
//map.setView({mapTypeId: Microsoft.Maps.MapTypeId.roadmap, zoom:19}); // 29Apr25 *TESTING
...
var fm=false; if (rmarkers.indexOf(rMxxyy)<0) { rmarkers += rMxxyy; fm = true;}  //12Sep Add: only show at-code once per tile; 18Oct mod: track to sub-tile
	  ...
	  markers.push(marker); map.entities.push(marker); console.log("push "+(markers.length-1)+" "+markers[markers.length-1].geometry)
 //qX = u.Eing.toString().slice(-4,-3); qY = u.Ning.toString().slice(-4,-3);  //ADD 02May25 KLUDGE
 addMarkerRow(Mxxyy, u);  //MOD 02May25, // 29Apr25
...
function showMarker(th) { //console.log("at showMarker '"+th.value+"' "+th.value.length)
  var u = {}, v = th.value, ij="", q;  //MOD 29Apr25
 q = "NESESWNW".indexOf(v.slice(0,2).toUpperCase()); //ADD 29Apr25 convert compass points to q-case
 if(q>=0) q = q +1; else {q = "NESW".indexOf(v.slice(0,1).toUpperCase()); if(q>=0) q = 2*q; } 
 if (q>=0) v = "23698741".charAt(q)+v.slice(q%2+1);  // NB: slice just after compass pt, may or m/n have @
 if (parseInt(v.charAt(0))>0) if (v.charAt(1)=="@") { v = v.charAt(0)+v.slice(2); }//console.log(th.value+"-->"+v)} //ADD elide @ in QCase strings
 ...
 if (v.length==6) {v = v.slice(0,4)+"."+v.slice(-2); th.value = v;}  // add 17Aug: treat all 6-digits as XXYY.xy
  ...
  u.Eing = uPosn.Eing; u.Ning = uPosn.Ning; //console.log(u)
 if (v.length>=4) {var i=0, j=0, xx = parseInt(v.slice(-4,-2)), yy = parseInt(v.slice(-2));
 if (v.length>=5) 
  if(!ij) { ij = parseInt(v.charAt(0)), i = ((ij%3?ij%3:3) -2), j = (ij<4?+1:ij<7?0:-1); //console.log (ij+" "+i+" "+j+"; "+xx+" "+yy)
 }}  //ADD 29Apr25
  switch(v.length) {
  ...
  //case 4: var ij = 5; u.Eing -= (u.Eing%100 -parseInt(v.slice(0,2))); u.Ning -= (u.Ning%100 -parseInt(v.slice(-2))); break;
	...
	case 5: var ij = parseInt(v.charAt(0)), i = ((ij%3?ij%3:3) -2), j = (ij<4?+1:ij<7?0:-1);
       u.Eing -= (u.Eing%1000 -(10*xx +5)); u.Ning -= (u.Ning%1000 -(10*yy +5)); break;  //ADD 29Apr25 --treat same as 4-digits: xx5yy5
...
//*  
 var delE=0, delN=0; //console.log(ij+" "+i+" "+j+"; "+uPosn.Eing+"_"+uPosn.Ning+"; "+Mtype) //add 24Aug24: ECM logic for 1x1 km tile
 if (i||j) { delE = 1000*i; delN = 1000*j;  //ADD 29Apr25
 } else {
 ...
 } //console.log (delE+" "+delN); //console.log(u)
 u.Eing += delE; u.Ning += delN; //console.log(u)
//*/
...
markerLatitude = elat; markerLongitude = elng; //console.log(elng+"_"+elat)  //Add 30Jul24
	} else if (params.has("Bing") || location.href.toLowerCase().indexOf("bing")>=0) {  //mod 09Sep
//console.log(ij +" "+elng+"_"+elat); //ADD 29Apr24
	//ij = ""; Mxxyy = "@"+uEing.slice(-3,-1)+uNing.slice(-3,-1)  //DEL 29Apr25 add 09Sep like 15Jul: hundreds * tens digits
 if (v.length<6) Mxxyy = ij+"@"+uEing.slice(-3,-1)+uNing.slice(-3,-1); //console.log(Mxxyy)//ADD 29Apr25
...
//console.log(fm + rmarkers)
    //marker = new Microsoft.Maps.Pushpin(new Microsoft.Maps.Location(elat, elng), {title:(fm?Mxxyy:""), subTitle:'', text:("."+(ij?ij:"55"))});  //12Sep //mod 09Sep like 08Sep
	 ...
	 markers.push(marker); map.entities.push(marker); //console.log("push "+markers.length);console.log(markers[markers.length-1].geometry) 
 //qX = u.Eing.toString().slice(-4,-3); qY = u.Ning.toString().slice(-4,-3);  //ADD 02May25 KLUDGE
 addMarkerRow(Mxxyy, u);  //MOD 02May25, ADD 22Apr25
 map.setView({center:marker.getLocation(), zoom:18}); // 02May25 *TESTING
  if (fm) { rect = rectb(u, uPosn.Zone); rects.push(rect); map.entities.push(rect);} //27Sep always, once per cell show 10x10m box
//handler = new  Microsoft.Maps.Events.addHandler(marker, 'rightclick', handleRtClick);  //*NB: not avail~
//but see: https://learn.microsoft.com/en-us/bingmaps/v8-web-control/map-control-concepts/event-examples/right-click-events-for-shapes
...
//function handleRtClick() { alert("clicked")}  // 30Oct add: Rtclick on marker: NOT Avialable in Bing
  function showMore() { //alert("at showMore "+lastCode) // 31Oct add: show 8 matched queens-case locations
	...
	for (var j=+1; j>=-1; j--)
	 for (var i=-1; i<=+1; i++) {
  var q = (5 +i -3*j);//console.log(f+" "+i+" "+j+" "+q)
  u.Eing = uEing +1000*i; u.Ning = uNing +1000*j;  // ADD 02May25
	//if (f||(i||j)) { //DEL 02May25 //04Nov include center if anchor //  skip center location, already marked
	  if (addMarkerRow(q+"@"+Mxxyy.slice(1,5), u)) {  //ADD 02May25 skip nearest tile location 
	 //u.Eing = uEing +1000*i; u.Ning = uNing +1000*j;  //DEL 02May25
	   LL = toLL(u, z); 
		if (Mxxyy.length>6)
      ...
      marker = new Microsoft.Maps.Pushpin(new Microsoft.Maps.Location(LL.lat, LL.lng), {title:"@"+Mxxyy.slice(1,5), subTitle:'', icon:'https://upload.wikimedia.org/wikipedia/commons/c/ca/1x1.png'});  //28Sep mod: no marker at 4-digit center
 ...
 //qX = u.Eing.toString().slice(-4,-3); qY = u.Ning.toString().slice(-4,-3);  //ADD 02May25 KLUDGE
 //addMarkerRow(q+"@"+Mxxyy.slice(1,5), u);  //DEL 02May25, MOD 29Apr: show @ADD 22Apr25
	  }
     } 
